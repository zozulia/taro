...
define('ownermail','korchenkosv@gmail.com');
//define('ownermail','roman.zozulia@gmail.com');
...
if ($email == ownermail){
	...
	$template = file_get_contents('./sender/letter2.html');
	$template = strtr($template,array('{--date--}'=>$date));
	send($email, $_POST['name'], $template);

	$template = file_get_contents('./sender/letter3.html');
	$template = strtr($template,array('{--date--}'=>$date));
	send($email, $_POST['name'], $template);
